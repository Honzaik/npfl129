...
parser.add_argument("--data_size", default=90, type=int, help="Data size")
...
def getAccLoss(data, target, weights):
    ...
    for i in range(data.shape[0]):
        prob = logistic.cdf(data[i] @ weights)
        res = 0
        lossCoef = prob
        if prob >= 0.5:
            res = 1
    ...
    for iteration in range(args.iterations):
        ...
        for j in range(rounds):
            ...
            for i in range(args.batch_size):
                ...
                sig = logistic.cdf(x_i.T @ weights)
